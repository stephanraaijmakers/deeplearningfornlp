...
def save_embedding(outputFile, weights, vocabulary): <1>
...
def create_vocabulary(vocabulary, sentences): <3>
  vocabulary["<unk>"]=0
...
def process_training_data(textFile,max_len): <4>
    data=[]
...
def process_test_data(textFile,vocab,max_len): <5>
    data=[]
    sentences = getLines(textFile)
    labels=[]
    create_vocabulary(vocab, sentences)
...
max_len=100
data,labels,vocab=process_training_data(sys.argv[1],max_len) <6>
test_data,test_labels=process_test_data(sys.argv[2],vocab,max_len)

model = Sequential()
embedding=Embedding(len(vocab), 100, input_length=max_len) <7>
model.add(embedding)
model.add(Flatten())
...
save_embedding("embedding_labeled.txt",embedding.get_weights()[0], vocab) <8>

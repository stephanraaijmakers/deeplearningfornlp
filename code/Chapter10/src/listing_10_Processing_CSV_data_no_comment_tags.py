...
def loadData(trainCSV, testCSV, valCSV, tokenizer): 
    ...
    y_train = label_encoder.fit_transform(pd.concat([train['label'],val['label']]))
    y_test = label_encoder.fit_transform(pd.concat([test['label'],val['label']]))
    y_val = label_encoder.fit_transform(pd.concat([train['label'],val['label']]))
    ...
    return [(train_input_ids,train_input_masks,train_segment_ids, train_labels),(test_input_ids,test_input_masks,test_segment_ids, test_labels),(val_input_ids,val_input_masks,val_segment_ids, val_labels)] 

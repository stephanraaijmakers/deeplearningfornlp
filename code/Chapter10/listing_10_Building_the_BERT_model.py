...
def buildBertModel():
  ...
  bert_inputs = [in_id, in_mask, in_segment] <2>

  bert_output = BertLayer(n_fine_tune_layers=10)(bert_inputs) <3>

  drop = keras.layers.Dropout(0.3)(bert_output) <4>
  ...
  model = tf.keras.models.Model(inputs=bert_inputs, outputs=pred) <6>

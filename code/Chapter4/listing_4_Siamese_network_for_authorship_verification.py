...
encoded_left = embedding(left_input) <1>
encoded_right = embedding(right_input)

nb_units=10 <2>

lstm = LSTM(nb_units) <3>
...
[CA]x[0], x[1]),output_shape=lambda x: (x[0][0], 1))(
[CA][left_output, right_output]) <4>

model = Model([left_input, right_input], [model_distance]) <5>

model.compile(loss='mean_squared_error', optimizer='adam',
[CA] metrics=['accuracy'])

model.fit([X_train_left, X_train_right], y_train_lr, batch_size=64,
[CA]nb_epoch=nb_epochs,

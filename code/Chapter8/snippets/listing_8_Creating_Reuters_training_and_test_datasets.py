...
for (topic1,topic2) in Stored:
    for (topic3,topic4) in Stored:
        if (topic1,topic2)==(topic3,topic4): <2>
            continue
        ...
        if len(x1_train)<300 or len(x2_train)<300: <8>
            continue

        min_train=min(len(x1_train),len(x2_train)) <9>
